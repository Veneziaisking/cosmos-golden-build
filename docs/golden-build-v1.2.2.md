...
**Status:** v1.2.2 complete and validated on reference Debian 13 N100 installation
...
**Note on `--system` flag:** The v1.2.2 installer does not use `useradd --system` when creating the `media` user, because `--system` is a hint to allocate from the system UID range (< 1000) and using it while also specifying `--uid 1001` would be contradictory. The security properties that matter — `nologin` shell, no password, dedicated group — are set explicitly.

### UID Migration Procedure (for systems upgraded from v1.2.1 or earlier)

If `media` already exists with a different UID/GID, the v1.2.2 installer will print migration guidance and exit. It will **not** automatically change an existing account's UID or GID.
...
sudo find /srv/cosmos /srv/cosmos-storage /srv/media /srv/backups \
...
| Installer output | `Golden Build v1.2.2 infrastructure provisioned — Cosmos initialization pending` |
...
| Installer output | `All checks passed. Golden Build v1.2.2 installation complete.` |
| Validator output | `RESULT: PASS` (exits 0) |

#### Two-Pass Sequence (fresh server)

```
[Pass 1]   sudo bash install-cosmos-golden-build-v1.2.2.sh
               ↳ State 1: infrastructure complete, config pending

[Validate] sudo bash validate-golden-build-v1.2.2.sh
...
[Pass 2]   sudo bash install-cosmos-golden-build-v1.2.2.sh
...
**`/etc/systemd/system/CosmosCloud.service.d/golden-build.conf`** — drop-in, unchanged from v1.1/v1.2/v1.2.1/v1.2.2 (the drop-in content is identical across all versions; v1.2.2 changes are in the installer user creation logic and validator checks):
...
The v1.2.2 installer output and Section 2 of this document both explicitly document this scope restriction in their migration guidance.
...
*End of Golden Build v1.2.2 documentation.*
