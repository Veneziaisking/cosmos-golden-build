...
**Status:** v1.2.1 complete and validated on reference Debian 13 N100 installation
...
**`/etc/systemd/system/CosmosCloud.service.d/golden-build.conf`** — drop-in, Golden Build v1.2.1 configuration (unchanged from v1.1/v1.2 — the drop-in content is identical; the v1.2.1 fix is in the installer sequencing):
...
| V32 | `/var/lib/docker` correctly classified as inactive orphan | `lsof +D /var/lib/docker`, engine-id comparison | PASS — engine-id in `/var/lib/docker/engine-id` differs from running engine; 0 open FDs |
...
| V34 | All v1.2 and earlier validation checks unaffected | Full `validate-golden-build-v1.2.1.sh` run | PASS — no regressions (same PASS counts as v1.2) |
...
After applying Golden Build v1.2, two directories may appear under `/var` that look like active state but are inert orphan artifacts. Both are safe to remove once the active Golden Build state is confirmed.

#### Root cause: `/var/lib/cosmos`

The Cosmos binary has `/var/lib/cosmos/` compiled in as its fallback config directory. This path is used only when `COSMOS_CONFIG_FOLDER` is not set in the process environment. The Golden Build sets this variable via the systemd drop-in, but a race existed in the v1.2 installer:

1. The installer wrote the drop-in and called `systemctl daemon-reload`.
2. The installer restarted Docker (`systemctl restart docker`).
3. The Docker restart severed Cosmos's Docker event stream, triggering an `EOF` error.
4. `Restart=always` auto-started Cosmos before the drop-in environment was guaranteed active in systemd's restart path.
5. That brief start used the compiled-in default `/var/lib/cosmos/`, creating `cosmos.config.json`, `cosmos.log`, and `cosmos.plain.log` there.
6. Docker came back up; Cosmos restarted a second time with the drop-in fully active, correctly using `/srv/cosmos/config/`.

The files at `/var/lib/cosmos/` are frozen since that one startup. No subsequent Cosmos process reads or writes them.

**Golden Build v1.2.1 fix:** The installer now explicitly stops CosmosCloud in Phase 7 **before** restarting Docker. `Restart=always` cannot fire during the Docker restart because the service is already in the stopped state. Cosmos is then started explicitly by the installer after Docker is stable, with the drop-in guaranteed active. The race is eliminated entirely.

#### Root cause: `/var/lib/docker`

The initial Docker daemon instance (before `daemon.json` was applied) started with the compiled-in default `data-root=/var/lib/docker`. It initialized the full directory structure there (buildkit databases, engine-id, network state, volumes metadata) before the installer wrote `daemon.json` with `data-root=/srv/docker` and restarted Docker. The replacement Docker daemon used `/srv/docker` exclusively and has never touched `/var/lib/docker` since.

**Evidence:** All files in `/var/lib/docker` carry the modification timestamp of the initial Docker start. The engine-id in `/var/lib/docker/engine-id` differs from the currently-running Docker engine's ID. `lsof +D /var/lib/docker` shows zero open file descriptors after Golden Build is applied.

#### Safe removal

Both paths are safe to remove after the validator confirms the system is in the correct state:

```bash
# 1. Confirm active state first
sudo docker info --format 'DockerRootDir: {{.DockerRootDir}}'
# must show: DockerRootDir: /srv/docker

sudo bash scripts/validate-golden-build-v1.2.1.sh
# must show: RESULT: PASS

# 2. Confirm no open file descriptors
sudo lsof +D /var/lib/cosmos 2>/dev/null || echo "clean"
sudo lsof +D /var/lib/docker 2>/dev/null || echo "clean"

# 3. Remove
sudo rm -rf /var/lib/cosmos
sudo rm -rf /var/lib/docker
```

`/var/lib/docker` will **not** be recreated by future Docker restarts as long as `daemon.json` remains in place with `data-root=/srv/docker`. The current Docker engine ignores `/var/lib/docker` entirely.

The v1.2.1 installer (Phase 9) and validator (Section 9) detect both paths automatically and report whether they are inactive (WARN — safe to remove) or actively used (FAIL — do not remove).
...
*End of Golden Build v1.2.1 documentation.*
