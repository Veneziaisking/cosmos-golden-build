#!/usr/bin/env bash
# =============================================================================
# Cosmos Cloud — Golden Build v1.2.2 Installer
# Target: Debian GNU/Linux 13 (trixie)
#
# Prepares a fresh or existing Debian 13 server for the Golden Build v1.2.2
...
#
# Usage: sudo bash install-cosmos-golden-build-v1.2.2.sh
# =============================================================================
set -euo pipefail
...
# CONSTANTS
# =============================================================================
readonly GOLDEN_BUILD_VERSION="1.2.2"
...
# across deployments. See docs/golden-build-v1.2.2.md Section 2 for rationale.
...
# This ensures reproducibility across deployments. See Section 2 of the docs.
#
...
if id "${MEDIA_USER}" &>/dev/null; then
    ...
    if [[ "${_ACTUAL_UID}" -eq "${MEDIA_UID}" && "${_ACTUAL_GID}" -eq "${MEDIA_GID}" ]]; then
        ...
        warn "Manual migration is required. See docs/golden-build-v1.2.2.md Section 2 for full details."
        printf "\n"
        ...
        printf "    sudo find /srv/cosmos /srv/cosmos-storage /srv/media /srv/backups \\\\\n"
...
# The exact drop-in content for Golden Build v1.2.2.
...
# were active. After a correct Golden Build application neither path should be in
# active use. This phase identifies them and reports whether they are safe to remove.
...
if "${COSMOS_INIT_PENDING}"; then
    ...
    printf "       sudo bash validate-golden-build-v1.2.2.sh\n\n"
else
