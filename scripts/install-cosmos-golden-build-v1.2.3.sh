#!/usr/bin/env bash
# =============================================================================
# Cosmos Cloud — Golden Build v1.2.3 Installer
# Target: Debian GNU/Linux 13 (trixie)
#
# Prepares a fresh or existing Debian 13 server for the Golden Build v1.2.3
# architecture. Safe to run multiple times — all phases are idempotent.
#
# What this script does:
#   1. Preflight: verifies OS, Docker, python3, backup dir
#   2. Creates the 'media' service user (UID=1001, GID=1001, nologin, docker group)
#      If 'media' already exists with 1001:1001, treats it as PASS.
#      If 'media' exists with a different UID/GID, provides migration guidance
#      and does NOT silently alter the account.
#   3. Creates the /srv directory layout with correct ownership/permissions,
#      including /srv/config (app bind-mount root) and /mnt group access.
#   4. Configures /etc/docker/daemon.json (data-root + log rotation)
#   5. Writes the CosmosCloud systemd hardening drop-in
#      ReadWritePaths=/srv /mnt /opt/cosmos (v1.2.3 — broader than v1.2.2)
#   6. Fixes DockerConfig.DefaultDataPath in cosmos.config.json if it exists
#   7. Applies changes: stop CosmosCloud → daemon-reload → Docker restart →
#      start CosmosCloud. The explicit stop-before-restart eliminates the v1.2
#      startup race that produced /var/lib/cosmos orphan artifacts.
#   8. Runs validation checks and reports PASS/FAIL
#   9. Reports orphan artifact status for /var/lib/cosmos and /var/lib/docker
#
...
#   - Automatically change an existing 'media' user's UID or GID
#   - Change /srv/docker ownership
#
# Usage: sudo bash install-cosmos-golden-build-v1.2.3.sh
# =============================================================================
set -euo pipefail
...
# CONSTANTS
# =============================================================================
readonly GOLDEN_BUILD_VERSION="1.2.3"
...
# Canonical UID/GID for the 'media' service account in Golden Build v1.2.2+.
# These are assigned explicitly rather than via --system to ensure reproducibility
# across deployments. See docs/golden-build-v1.2.3.md Section 2 for rationale.
readonly MEDIA_UID=1001
readonly MEDIA_GID=1001
...
readonly SRV_CONFIG="/srv/config"
...
# Golden Build v1.2.2+ assigns 'media' a fixed UID=1001 and GID=1001.
# This ensures reproducibility across deployments.
#
# If 'media' already exists with 1001:1001 → PASS (idempotent).
# If 'media' exists with a different UID/GID → print migration guidance and flag
#   the check as failed. The installer will NOT silently alter an existing account.
# If 'media' does not exist → create with UID=1001, GID=1001.

if id "${MEDIA_USER}" &>/dev/null; then
    _ACTUAL_UID="$(id -u "${MEDIA_USER}")"
    _ACTUAL_GID="$(id -g "${MEDIA_USER}")"

    if [[ "${_ACTUAL_UID}" -eq "${MEDIA_UID}" && "${_ACTUAL_GID}" -eq "${MEDIA_GID}" ]]; then
        ok "User '${MEDIA_USER}' exists with correct UID=${MEDIA_UID} GID=${MEDIA_GID}"
        ...
        warn "User '${MEDIA_USER}' exists with UID=${_ACTUAL_UID} GID=${_ACTUAL_GID} — expected ${MEDIA_UID}:${MEDIA_GID}"
        warn "This installer does not automatically change an existing UID or GID."
        warn "Manual migration is required. See docs/golden-build-v1.2.3.md Section 2 for full details."
        printf "\n"
        info "Migration quick reference (all commands require root):"
        printf "\n"
        printf "    # Step 1: Stop all services\n"
        printf "    sudo systemctl stop CosmosCloud docker docker.socket containerd\n\n"
        printf "    # Step 2: Change the primary group GID\n"
        printf "    sudo groupmod -g %d %s\n\n" "${MEDIA_GID}" "${MEDIA_USER}"
        printf "    # Step 3: Change the user UID and primary GID\n"
        printf "    sudo usermod -u %d -g %d %s\n\n" "${MEDIA_UID}" "${MEDIA_GID}" "${MEDIA_USER}"
        printf "    # Step 4: Rechown media-owned files (do NOT include /srv/docker)\n"
        printf "    sudo find /srv/cosmos /srv/cosmos-storage /srv/config /srv/media /srv/backups \\\\\n"
        printf "         /opt/cosmos /home/media \\\\\n"
        printf "         \\( -uid %d -o -gid %d \\) \\\\\n" "${_ACTUAL_UID}" "${_ACTUAL_GID}"
        printf "         -exec chown %d:%d {} +\n\n" "${MEDIA_UID}" "${MEDIA_GID}"
        printf "    # Step 5: DO NOT rechown /srv/docker/volumes\n"
        printf "    #    MongoDB volumes (cosmos-mongo-*/_data) are owned 999:999 internally.\n"
        printf "    #    The mongo:8 entrypoint enforces this on every container start.\n"
        printf "    #    Rechowning them is futile and risks transient I/O errors.\n\n"
        printf "    # Step 6: Restart services in dependency order\n"
        printf "    sudo systemctl start containerd docker.socket docker CosmosCloud\n\n"
        printf "    # Step 7: Re-run this installer to complete and validate\n"
        printf "    sudo bash %s\n\n" "${SCRIPT_NAME}"
        MEDIA_UID_MISMATCH=true
    fi
else
    info "Creating user '${MEDIA_USER}' (UID=${MEDIA_UID}, GID=${MEDIA_GID})"

    # Create the primary group with explicit GID before creating the user.
    # We do not use useradd --user-group because that may not assign a predictable GID.
    if ! getent group "${MEDIA_USER}" &>/dev/null; then
        groupadd --gid "${MEDIA_GID}" "${MEDIA_USER}"
        ok "Created group '${MEDIA_USER}' (GID=${MEDIA_GID})"
    fi
...
# Docker group membership (checked regardless of UID/GID state)
...
# Cosmos Marketplace apps that specify absolute bind-mount paths like
# /srv/config/<appname> will find this directory pre-created with correct
# ownership so mkdir succeeds under ProtectSystem=strict.
ensure_dir "${SRV_CONFIG}"         "media:media" "2775"
...
# The media group must have write permission so Cosmos can create bind-mount
# directories for media-stack applications (e.g. Plex, Jellyfin, Sonarr).
# Root retains ownership; only the group and mode are adjusted.
_MNT_GRP="$(stat -c '%G' /mnt 2>/dev/null || echo "")"
_MNT_MOD="$(stat -c '%a' /mnt 2>/dev/null || echo "")"
if [[ "${_MNT_GRP}" == "${MEDIA_USER}" && "${_MNT_MOD}" == "775" ]]; then
    ok "/mnt  →  group=${MEDIA_USER}  mode=775 (already correct)"
else
    chgrp "${MEDIA_USER}" /mnt
    chmod 775 /mnt
    ok "/mnt  →  group=${MEDIA_USER}  mode=775"
fi
...
# The exact drop-in content for Golden Build v1.2.3.
# Key change from v1.2.2: ReadWritePaths broadened to /srv /mnt /opt/cosmos
# for Marketplace app compatibility. See docs/golden-build-v1.2.3.md Section 9.12.
...
# ── Step 1: Stop CosmosCloud before Docker restart ────────────────────────────
# When daemon.json changes, Docker must be restarted. Without an explicit stop
# first, systemd's Restart=always auto-starts CosmosCloud the moment Docker
# severs the event stream (EOF) — potentially before daemon-reload has activated
# the drop-in's COSMOS_CONFIG_FOLDER setting. That window caused Cosmos to fall
# back to its compiled-in default /var/lib/cosmos/, leaving orphan files behind.
# Stopping CosmosCloud here ensures it cannot start until we start it explicitly
# in Step 4, with the drop-in guaranteed active.
if "${DOCKER_DAEMON_CHANGED}"; then
    if systemctl is-active --quiet CosmosCloud 2>/dev/null; then
        info "Stopping CosmosCloud before Docker restart (prevents startup race)..."
        systemctl stop CosmosCloud
        COSMOS_WAS_RUNNING=true
...
# ── Step 2: Reload systemd if the drop-in changed ────────────────────────────
...
# ── Step 3: Restart Docker if daemon.json changed ────────────────────────────
if "${DOCKER_DAEMON_CHANGED}"; then
...
# ── Step 4: Start or restart CosmosCloud ─────────────────────────────────────
# Re-check live state — Phase 6 or Step 1 above may have stopped it.
...
if "${COSMOS_SERVICE_EXISTS}"; then
    if "${DOCKER_DAEMON_CHANGED}" || "${DROPIN_CHANGED}"; then
        if "${COSMOS_SERVICE_ACTIVE}"; then
            info "Restarting CosmosCloud (configuration changed)..."
            systemctl restart CosmosCloud
            ok "CosmosCloud restarted"
        elif "${COSMOS_WAS_RUNNING}"; then
            info "Starting CosmosCloud (was stopped for configuration change)..."
            systemctl start CosmosCloud
            ok "CosmosCloud started"
        else
            info "CosmosCloud is not running — configuration will take effect on next start"
        fi
    elif "${COSMOS_WAS_RUNNING}"; then
        # Was stopped in Phase 6 for a config-only change (no daemon or drop-in change)
        if ! "${COSMOS_SERVICE_ACTIVE}"; then
            info "Starting CosmosCloud (was stopped for config-only change)..."
            systemctl start CosmosCloud
            ok "CosmosCloud started"
        fi
    fi
...
if id "${MEDIA_USER}" &>/dev/null; then
    ...
    _V_UID="$(id -u "${MEDIA_USER}")"
    _V_GID="$(id -g "${MEDIA_USER}")"
    if [[ "${_V_UID}" -eq 0 ]]; then
        check_fail "media UID is 0 (root) — Cosmos must not run as root"
    elif [[ "${_V_UID}" -eq "${MEDIA_UID}" && "${_V_GID}" -eq "${MEDIA_GID}" ]]; then
        check_pass "media UID=${MEDIA_UID} GID=${MEDIA_GID}"
    else
        check_fail "media UID=${_V_UID} GID=${_V_GID} — expected ${MEDIA_UID}:${MEDIA_GID} (migration required — see Phase 2 output)"
    fi
else
...
# /mnt — group must be 'media', mode must be 775
_V_MNT_GRP="$(stat -c '%G' /mnt 2>/dev/null || echo "")"
_V_MNT_MOD="$(stat -c '%a' /mnt 2>/dev/null || echo "")"
if [[ "${_V_MNT_GRP}" == "${MEDIA_USER}" && "${_V_MNT_MOD}" == "775" ]]; then
    check_pass "/mnt  (root:${MEDIA_USER} ${_V_MNT_MOD})"
else
    check_fail "/mnt: expected group=${MEDIA_USER} mode=775, got group=${_V_MNT_GRP} mode=${_V_MNT_MOD}"
fi
...
if [[ -f "${DROPIN_FILE}" ]]; then
    ...
    if grep -qE "^ReadWritePaths=.*/srv" "${DROPIN_FILE}" && \
       grep -qE "^ReadWritePaths=.*/mnt" "${DROPIN_FILE}" && \
       grep -qE "^ReadWritePaths=.*/opt/cosmos" "${DROPIN_FILE}"; then
        check_pass "ReadWritePaths includes /srv, /mnt, /opt/cosmos"
    else
        check_fail "ReadWritePaths is missing one or more required entries (/srv /mnt /opt/cosmos)"
    fi
else
...
if systemctl is-active --quiet CosmosCloud 2>/dev/null; then
    ...
    if [[ -n "${COSMOS_PID}" && "${COSMOS_PID}" != "0" ]]; then
        ...
        CONFIG_ENV="$(sudo cat "/proc/${COSMOS_PID}/environ" 2>/dev/null | tr '\0' '\n' | grep '^COSMOS_CONFIG_FOLDER=' | cut -d= -f2- || echo "")"
...
# =============================================================================
# PHASE 9: ORPHAN ARTIFACT CHECK
# =============================================================================
step "Phase 9: Orphan Artifact Check"

_ARTIFACTS_PRESENT=false
_ARTIFACTS_SAFE=true
...
if [[ -d /var/lib/cosmos ]]; then
    ...
    if [[ "${_VLC_FDS}" -gt 0 ]]; then
        check_fail "/var/lib/cosmos has ${_VLC_FDS} open file descriptor(s) — active use detected"
        lsof +D /var/lib/cosmos 2>/dev/null | tail -n +2 | \
            awk '{printf "  PID=%-6s CMD=%-20s FD=%s  %s\n", $2, $1, $4, $NF}'
        _ARTIFACTS_SAFE=false
    else
        warn "/var/lib/cosmos exists — no open file descriptors"
        warn "  → One-time installation artifact; safe to remove after validation"
    fi
else
    ok "/var/lib/cosmos: not present (clean)"
fi
...
if [[ -d /var/lib/docker ]]; then
    ...
    if [[ "${_VLD_ROOT}" != "/srv/docker" ]]; then
        check_fail "/var/lib/docker exists and Docker Root Dir is '${_VLD_ROOT}' (expected /srv/docker)"
        _ARTIFACTS_SAFE=false
    else
        _VLD_FDS="$(lsof +D /var/lib/docker 2>/dev/null | tail -n +2 | wc -l)"
        if [[ "${_VLD_FDS}" -gt 0 ]]; then
            check_fail "/var/lib/docker has ${_VLD_FDS} open file descriptor(s) despite Docker Root Dir=/srv/docker"
    ...
    ok "/var/lib/docker: not present (clean)"
fi

# ── Cleanup guidance ──────────────────────────────────────────────────────────
if "${_ARTIFACTS_PRESENT}" && "${_ARTIFACTS_SAFE}"; then
    printf "\n"
    info "Orphan artifacts confirmed inactive — safe to remove:"
    [[ -d /var/lib/cosmos ]] && printf "    sudo rm -rf /var/lib/cosmos\n"
    [[ -d /var/lib/docker ]] && printf "    sudo rm -rf /var/lib/docker\n"
fi
...
if "${COSMOS_INIT_PENDING}"; then
    ...
    printf "       sudo bash validate-golden-build-v1.2.3.sh\n\n"
else
