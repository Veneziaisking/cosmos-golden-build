#!/usr/bin/env bash
# =============================================================================
# Cosmos Cloud — Golden Build v1.2 Validation Script
#
# Checks that the live system matches the Golden Build v1.2 specification.
# Runs all checks and reports PASS or FAIL for each. Exits 0 if all pass,
# exits 1 if any fail.
#
# Does not modify any system state.
#
# Usage: sudo bash validate-golden-build-v1.2.sh
# =============================================================================
set -uo pipefail
...
printf "\n${BOLD}Cosmos Cloud — Golden Build v1.2 Validation${NC}\n"
printf "%-50s  %s\n" "$(date)" "$(hostname)"
printf '%0.s─' {1..60}
printf '\n'
...
if id "${MEDIA_USER}" &>/dev/null; then
    ...
    if [[ "${MEDIA_UID}" -lt 1000 ]]; then
        pass "media UID is in system range (< 1000)"
    else
        fail "media UID ${MEDIA_UID} is in user range (>= 1000) — useradd --system was not used"
    fi
...
if [[ -f "${DROPIN_FILE}" ]]; then
    ...
    check_contains "ReadWritePaths"             "${DROPIN_FILE}" "^ReadWritePaths=.*\/srv\/cosmos.*\/srv\/cosmos-storage.*\/opt\/cosmos"
else
...
if [[ "${FAIL_COUNT}" -gt 0 ]]; then
    ...
    printf "  sudo bash install-cosmos-golden-build-v1.2.sh\n\n"
    exit 1
elif "${COSMOS_INIT_PENDING}"; then
    printf "${YELLOW}${BOLD}RESULT: PENDING${NC} — Golden Build v1.2 infrastructure in place; Cosmos initialization required.\n\n"
    ...
    printf "${GREEN}${BOLD}RESULT: PASS${NC} — system matches Golden Build v1.2 specification.\n\n"
    exit 0
fi
